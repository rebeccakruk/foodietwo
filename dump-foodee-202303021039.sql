...
BEGIN
	IF order_id_input IS NULL THEN
		SELECT o.client_id, o.created_at, o.is_cancelled, o.is_completed, o.is_confirmed, omi.menu_item_id, o.id, o.restaurant_id, COUNT(o.client_id)
		FROM orders o
...
/*!50003 SET sql_mode              = 'IGNORE_SPACE,STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`innotech`@`%` PROCEDURE `new_restaurant`(name_input VARCHAR(100), address_input VARCHAR(100), banner_url_input VARCHAR(500), bio_input VARCHAR(500), city_input VARCHAR(100), email_input VARCHAR(100), phone_num_input CHAR(12), password_input VARCHAR(100), profile_url_input VARCHAR(500), token VARCHAR(45))
    MODIFIES SQL DATA
BEGIN
	INSERT INTO restaurant(name, address, banner_url, bio, city, email, phone_num, password, profile_url)
		VALUES(name_input, address_input, banner_url_input, bio_input, city_input, email_input, phone_num_input, password_input, profile_url_input);
...
/*!50003 SET sql_mode              = 'IGNORE_SPACE,STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`innotech`@`%` PROCEDURE `update_order`(client_id_input INT(10)UNSIGNED, restaurant_id_input INT(10)UNSIGNED, order_id_input INT(10)UNSIGNED)
    MODIFIES SQL DATA
BEGIN
	IF restaurant_id_input IS NULL THEN
		UPDATE orders
		SET is_cancelled = is_cancelled_input;
			SELECT o.is_cancelled
			FROM orders o
			WHERE o.client_id = client_id_input;
	ELSE
		UPDATE orders
		SET is_completed = is_completed_input, is_confirmed = is_confirmed_input;
			SELECT o.is_completed, o.is_confirmed
			FROM orders o
			WHERE o.restaurant_id = restaurant_id_input;
	END IF;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
-- Dump completed on 2023-03-02 10:39:56
